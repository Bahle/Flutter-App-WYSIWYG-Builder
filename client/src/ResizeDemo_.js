...
const ResizeDemo = ({id, stageId, type, x, y, width, height, key, isFocused = false}) => {
  /*let [offset, setOffset] = React.useState(0);
  setTimeout(() => {
  	setOffset(window.$('#' + stageId).offset())
  }, 1000)*/

  const [coordinate, setCoordinate] = React.useState({ x: x || 0, y: y || 0, width: width || 30, height: height || 30, isFocused, id, type, stageId, key: id });
